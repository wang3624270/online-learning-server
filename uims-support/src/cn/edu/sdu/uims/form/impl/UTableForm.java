...
public class UTableForm extends UForm{
	...
	public void clearSelectItemsMark(){
		if(items == null || items.length == 0)
			return;
		if(!(items[0] instanceof USFormI)) 
			return ; 
		USFormI mi;
		for(int i = 0; i <items.length;i++){
			mi = (USFormI)items[i];
	...
	public List getModifyItemList(){
		if(items == null || items.length == 0) 
			return null;
		List list = new ArrayList();
		UFormModifyStatusI f;
		for(int i = 0; i < items.length;i++) {
			if(!(items[i] instanceof UFormModifyStatusI))
				continue;
			f = (UFormModifyStatusI)items[i];
			if(f.isModify()) 
				list.add(f);
		}
		return list;
