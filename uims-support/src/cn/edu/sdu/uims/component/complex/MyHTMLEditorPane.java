...
public class MyHTMLEditorPane extends JPanel {
	/**
    * 
    */
	...
	private void createEditorActions() {
		...
		wysPopupMenu = ActionUIFactory.getInstance().createPopupMenu(
				editActions);
		wysPopupMenu.addSeparator();
		wysPopupMenu.add(objectPropertiesAction);
		srcPopupMenu = ActionUIFactory.getInstance().createPopupMenu(
				editActions);
	...
	private void createFormatToolBar(ActionList blockActs,
		...
		final JButton fontSizeButton = new JButton(UIUtils.getIcon(UIUtils.X16,
				"fontsize.png"));
	...
	private JEditorPane createWysiwygEditor() {
		...
		ed.setEditorKitForContentType("text/html;charset=utf-8",
				new WysiwygHTMLEditorKit());

		ed.setContentType("text/html;charset=utf-8");

		// ed.putClientProperty(JEditorPane.HONOR_DISPLAY_PROPERTIES,
		// Boolean.TRUE);
		// ed.putClientProperty("charset", "utf-8");
		insertHTML(ed, "<p></p>", 0);
	...
	public String getText() {
		...
		String t = Entities.HTML40.unescapeUnknownEntities(topText);
		return t;
	}
	...
	private void updateState() {
		...
		actionList.putContextValueForAll(HTMLTextEditAction.EDITOR,
				focusedEditor);
	...
	private class ChangeTabAction extends DefaultAction {
        ...
        */
		...
		public ChangeTabAction(int tab) {
			...
			this.tab = tab;
			putValue(ActionManager.BUTTON_TYPE,
