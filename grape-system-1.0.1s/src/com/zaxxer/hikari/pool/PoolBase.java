...
abstract class PoolBase {
		...
		if (connection != null) {
			try {
				...
				try {
					...
					connection.close(); // continue with the close even if
										// setNetworkTimeout() throws
				}
			} catch (Throwable e) {
	...
	void resetConnectionState(final Connection connection, final ProxyConnection proxyConnection, final int dirtyBits)
			throws SQLException {
		int resetBits = 0;
	...
	private void checkDriverSupport(final Connection connection) throws SQLException {
		if (!isValidChecked) {
			try {
				...
				LOGGER.error(
						"{} - Failed to execute" + (isUseJdbc4Validation ? " isValid() for connection, configure" : "")
			...
			try {
				...
				LOGGER.warn("{} - Default transaction isolation level detection failed ({}).", poolName,
						e.getMessage());
	 ...
	 *
	 * @param statement
	 *            a statement to set the query timeout on
	 * @param timeoutSec
	...
	/**
	 * Set the network timeout, if <code>isUseNetworkTimeout</code> is
	 * <code>true</code> and the driver supports it. Return the pre-existing
	 * value of the network timeout.
	 *
	 * @param connection
	 *            the connection to set the network timeout on
	 * @param timeoutMs
		...
		if (isNetworkTimeoutSupported != FALSE) {
			try {
				...
				if (isNetworkTimeoutSupported == UNINITIALIZED) {
					...
					LOGGER.info("{} - Driver does not support get/set network timeout for connections. ({})", poolName,
							e.getMessage());
					if (validationTimeout < SECONDS.toMillis(1)) {
						LOGGER.warn(
								"{} - A validationTimeout of less than 1 second cannot be honored on drivers without setNetworkTimeout() support.",
								poolName);
					} else if (validationTimeout % SECONDS.toMillis(1) != 0) {
						LOGGER.warn(
								"{} - A validationTimeout with fractional second granularity cannot be honored on drivers without setNetworkTimeout() support.",
								poolName);
					}
				}
			}
	...
	/**
	 * Set the network timeout, if <code>isUseNetworkTimeout</code> is
	 * <code>true</code> and the driver supports it.
	 *
	 * @param connection
	 *            the connection to set the network timeout on
	 * @param timeoutMs
	 *            the number of milliseconds before timeout
	 * @throws SQLException
	 *             throw if the connection.setNetworkTimeout() call throws
	 */
	 ...
	 *            the SQL to execute
	 * @param isCommit
	 *            whether to commit the SQL after execution or not
	 * @throws SQLException
	 *             throws if the init SQL execution fails
	 */
	private void executeSql(final Connection connection, final String sql, final boolean isCommit) throws SQLException {
		if (sql != null) {
			try (Statement statement = connection.createStatement()) {
				// connection was created a few milliseconds before, so set
				// query timeout is omitted (we assume it will succeed)
	...
	private void createNetworkTimeoutExecutor(final DataSource dataSource, final String dsClassName,
			final String jdbcUrl) {
		// Temporary hack for MySQL issue:
		// http://bugs.mysql.com/bug.php?id=75615
		if ((dsClassName != null && dsClassName.contains("Mysql")) || (jdbcUrl != null && jdbcUrl.contains("mysql"))
	...
	/**
	 * This will create a string for debug logging. Given a set of "reset bits",
	 * this method will return a concatenated string, for example:
	 *
	 * Input : 0b00110 Output: "autoCommit, isolation"
	 *
	 * @param bits
	 *            a set of "reset bits"
	...
	/**
	 * Special executor used only to work around a MySQL issue that has not been
	 * addressed. MySQL issue: http://bugs.mysql.com/bug.php?id=75615
	 */
	...
	/**
	 * A class that delegates to a MetricsTracker implementation. The use of a
	 * delegate allows us to use the NopMetricsTrackerDelegate when metrics are
	 * disabled, which in turn allows the JIT to completely optimize away to
	 * callsites to record metrics.
	 */
	...
	/**
	 * A no-op implementation of the IMetricsTrackerDelegate that is used when
	 * metrics capture is disabled.
	 */
