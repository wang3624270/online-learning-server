 ...
 */
final public class PoolEntry implements IConcurrentBagEntry {
	private static final Logger LOGGER = LoggerFactory.getLogger(PoolEntry.class);
	private static final AtomicIntegerFieldUpdater<PoolEntry> stateUpdater;

	static final Comparator<PoolEntry> LASTACCESS_REVERSE_COMPARABLE;

	public StackTraceElement[] eee;

	Connection connection;
	long lastAccessed;
	public long lastBorrowed;
	...
	static {
		LASTACCESS_REVERSE_COMPARABLE = (entryOne, entryTwo) -> Long.compare(entryTwo.lastAccessed,
				entryOne.lastAccessed);
	 ...
	 *
	 * @param endOfLife
	 *            this PoolEntry/Connection's end of life
	...
	public void printbug() {
		if (eee != null) {
			...
			for (i = 0; i < eee.length; i++) {
				cc = eee[i].toString();
				if (cc.indexOf("com.badminton") >= 0 || cc.indexOf("cn.sdu.edu") >= 0)
					LOGGER.info(cc);
