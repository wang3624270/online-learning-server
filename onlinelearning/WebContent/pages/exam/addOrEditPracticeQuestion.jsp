...
<body>
	...
	<div class="easyui-panel" >
		<a class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-back'" onclick="doReturn('elearningPracticeManage.jsp')"></a>
		<a class="easyui-linkbutton" data-options="plain:true" onclick="doReturn('elearningPracticeManage.jsp')">在线练习管理</a>\
		<a class="easyui-linkbutton" data-options="plain:true" onclick="refresh()" id="currentState">在线练习试题</a>
    </div>
	<div class="easyui-panel" title="在线练习试题" style="width:99%;height:95%;padding:10px 30px;" data-options="iconCls: 'icon-save'">
...
$(document).ready(function() {
	initQuestionList();
	var practiceIdS=getUrlValue("practiceId");
	var practiceId=parseInt(practiceIdS);
	data3={practiceId:practiceId};
	Kelp.jsonPost("../../func/examBean/getPracticeInfo", data3, function(result) {
		if(result.re=="1"){
			$("#currentState").html("在线练习试题(在线练习名称："+result.data.practiceTitle+")");
		}else{
...
function initQuestionList(){
	$("#examList").empty();
	var practiceId=getUrlValue("practiceId");
	data={practiceId:practiceId};
	Kelp.jsonPost("../../func/examBean/getPracticeQustionList", data, function(result) {
		if(result.re=="1"){
...
function submitRadio(){
	...
	if(question=="" ){
		...
		if(questionId==""){
			data2={practiceId:practiceId,question:question,optionList:optionList,analysis:analysis};
			Kelp.jsonPost("../../func/examBean/addPracticeRadioQuestion", data2, function(result) {
				if(result.re=="1"){
					initQuestionList();
					$('#radio').form('clear');
				}else{
			...
			data2={practiceId:practiceId,questionId:questionId,question:question,optionList:optionList,analysis:analysis};
...
function subMultiple(){
	...
	if(question=="" ){
		...
		if(questionId==""){
			data2={practiceId:practiceId,question:question,optionList:optionList,analysis:analysis};
			Kelp.jsonPost("../../func/examBean/addPracticeMultipleQuestion", data2, function(result) {
				if(result.re=="1"){
					initQuestionList();
					$('#multiple').form('clear');
				}else{
...
function deleteExamQuestionR(id){
	data3={id:id};
	Kelp.jsonPost("../../func/examBean/deletePracticeQuestionR", data3, function(result) {
		if(result.re=="1"){
			initQuestionList();
		}else{
			$.messager.alert('提示',result.data,'info');
...
function adjustExamQuestion(id,direction){
	...
	Kelp.jsonPost("../../func/examBean/adjustPracticeQuestion", data3, function(result) {
		if(result.re=="1"){
			initQuestionList();
		}else{
			$.messager.alert('提示',result.data,'info');
...
function initHouseQuestionList(){
	...
	var practiceId=getUrlValue("practiceId");
	data={practiceId:practiceId};
	Kelp.jsonPost("../../func/examBean/getHouseQustionListOfPractice", data, function(result) {
		if(result.re=="1"){
...
function selectQuestion(questionId){
	...
	var practiceId=getUrlValue("practiceId");
	data={practiceId:practiceId,questionId:questionId};
	Kelp.jsonPost("../../func/examBean/addQuestionFormHouseOfPractice", data, function(result) {
		if(result.re=="1"){
			initQuestionList();
		}else{
			$.messager.alert('提示',result.data,'info');
