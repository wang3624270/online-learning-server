...
<body>
	...
	<div class="easyui-panel" >
		...
		<a href="#" class="easyui-linkbutton" data-options="plain:true" onclick="refresh()">添加课程</a>
    </div>
		...
		<form id="newcourse" class="easyui-form" method="post" data-options="novalidate:true">
		    <div style="height:205px;margin-bottom:20px;">
		        ...
		        <div style="float:right;width:350px;height:100%;margin-left:-350px;">
		             <div style="width:100%;height:100%"><img id="img" src="<%=request.getContextPath()%>/webFramework/common/image/default_cover_img.jpg" alt='课程封面' width='350px' height='205px' /></div>
		        </div>
		    </div>
...
$(document).ready(function() {
	...
	data3={type:"KCLXM"};
	Kelp.jsonPost("../../func/courseBean/getListOptionByType", data3, function(result) {
		if(result.re=="1"){
			var json = [];
			var row0 = {};
			row0.id= "";
...
function submitForm(){
	...
	if(courseName=="" || enlishName=="" || courseNum==""  || teachGroup=="" ||book=="" || collegeId=="" || courseType==""){
		...
		data2={courseType:courseType,courseName:courseName,enlishName:enlishName,courseNum:courseNum,collegeId:collegeId,teachGroup:teachGroup,book:book,reference:reference,briefIntroduction:briefIntroduction};
		Kelp.jsonPost("../../func/courseBean/addCourse", data2, function(result) {
			if(result.re=="1"){
                //封面上传
		        ...
		        formData.append("courseId",result.data.courseId);
		        $.ajax({
		            type:"post",
		            url:uploadUrl,
		            data:formData,
