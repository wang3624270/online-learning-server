...
$.fn.extend({
    "initUpload":function(opt) {
        ...
        if(uploadId==null||uploadId==""){
        	$.messager.alert('提示',"要设定一个id!",'info');
            //alert("要设定一个id!");
        }
...
var uploadTools = {
    ...
    "addFileList":function(fileList,opt){
        ...
        if(fileNumber+fileList.length>opt.maxFileNumber){
            ...
            $.messager.alert('提示',"最多只能上传"+opt.maxFileNumber+"个文件",'info');
            return;
        }
        ...
        for(var i=0;i<fileList.length;i++){
            ...
            if(uploadTools.fileIsExit(fileList[i],opt)){
            	...
            	$.messager.alert('提示',"文件（"+fileList[i].name+"）已经存在！",'info');
                continue;
            }
            if(opt.rememberUpload){
                if(uploadTools.fileIsHaveUpload(fileList[i],opt)){
                	$.messager.alert('提示',"文件（"+fileList[i].name+"）已经上传过，不能再次上传！",'info');
            ...
            if(fileSizeLimit!=-1&&fileList[i].size>(fileSizeLimit*1000)){
            	$.messager.alert('提示',"文件（"+fileList[i].name+"）超出了大小限制！请控制在"+fileSizeLimit+"KB内",'info');
            ...
            if(fileTypeArray=="*"||uploadTools.isInArray(fileTypeStr,fileTypeArray)){
                ...
                $.messager.alert('提示',"不支持该格式文件上传:"+fileList[i].name,'info');
            }
        }
    ...
    "uploadFile":function(opt){
        ...
        if(fileNumber<=0){
            //alert("没有文件，不支持上传");
            $.messager.alert('提示',"没有文件，不支持上传",'info');
            return;
        }
    ...
    "flushRememberFile":function(fileList,opt){
        if(opt.rememberUpload){
            if(opt.rememberFile==null||opt.rememberFile==""||opt.rememberFile.length==0){
                ...
                $.messager.alert('提示',opt.rememberFile.length,'info');
